...
class RolloutStorage:
    def __init__(self, num_envs, num_transitions_per_env, actor_obs_shape, critic_obs_shape, privileged_obs_shape, actions_shape, device):
        self.device = device
        ...
        self.rewards = np.zeros([num_transitions_per_env, num_envs, 2], dtype=np.float32)
        ...
        # For PPO
        self.actions_log_prob = np.zeros([num_transitions_per_env, num_envs, 2], dtype=np.float32)
        self.values = np.zeros([num_transitions_per_env, num_envs, 2], dtype=np.float32)
        self.returns = np.zeros([num_transitions_per_env, num_envs, 2], dtype=np.float32)
        self.advantages = np.zeros([num_transitions_per_env, num_envs, 2], dtype=np.float32)
    ...
    def add_transitions(self, actor_obs, critic_obs, actions, mu, sigma, rewards, arm_rewards, dones, actions_log_prob):
        ...
        self.rewards[self.step] = np.stack((rewards, arm_rewards), axis=-1).reshape(-1, 2)
    ...
    def mini_batch_generator_shuffle(self, num_mini_batches):
        ...
        for indices in BatchSampler(SubsetRandomSampler(range(batch_size)), mini_batch_size, drop_last=True):
            ...
            values_batch = self.values_tc.view(-1, 2)[indices]
            returns_batch = self.returns_tc.view(-1, 2)[indices]
            old_actions_log_prob_batch = self.actions_log_prob_tc.view(-1, 2)[indices]
            advantages_batch = self.advantages_tc.view(-1, 2)[indices]
    ...
    def mini_batch_generator_inorder(self, num_mini_batches):
        ...
        for batch_id in range(num_mini_batches):
            yield self.actor_obs_tc.view(-1, *self.actor_obs_tc.size()[2:])[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                ...
                self.values_tc.view(-1, 2)[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                self.advantages_tc.view(-1, 2)[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                self.returns_tc.view(-1, 2)[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size], \
                self.actions_log_prob_tc.view(-1, 2)[batch_id*mini_batch_size:(batch_id+1)*mini_batch_size]