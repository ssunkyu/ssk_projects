...
class PPO:
    def __init__(self,
                 ...
                 device='cpu',
                 shuffle_batch=True,
                 mixing_schedule=[1., 0., 3000.],
                 ):

        # PPO components
        self.actor = actor
        self.critic = critic
        self.storage = RolloutStorage(num_envs, num_transitions_per_env, actor.obs_shape, critic.obs_shape, actor.priv_shape, actor.action_shape, device)
    ...
    def step(self, value_obs, rews, arm_rews, dones):
        self.storage.add_transitions(self.actor_obs, value_obs, self.actions, self.actor.action_mean, self.actor.distribution.std_np, rews, arm_rews, dones,
    ...
    def _train_step(self, log_this_iteration):
        ...
        for epoch in range(self.num_learning_epochs):
            for actor_obs_batch, critic_obs_batch, actions_batch, old_sigma_batch, old_mu_batch, current_values_batch, \
