...
int main(int argc, char *argv[]) {
  ...
  EigenVec arm_reward(config["num_envs"].template As<int>(), 1);
  ...
  Eigen::Ref<EigenVec> arm_reward_ref(arm_reward);
  Eigen::Ref<EigenBoolVec> dones_ref(dones);

  vecEnv.reset();
  vecEnv.step(action_ref, reward_ref, arm_reward_ref, dones_ref);
