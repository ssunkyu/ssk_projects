...
class VectorizedEnvironment {
 ...
 public:
  ...
  void init() {
    ...
    armrewardInformation_.reserve(num_envs_);
    for (int i = 0; i < num_envs_; i++) {
      ...
      armrewardInformation_.push_back(environments_.back()->getarmRewards().getStdMap());
    }
...
#pragma omp parallel for schedule(auto)
  ...
  void step(Eigen::Ref<EigenRowMajorMat> &action,
            ...
            Eigen::Ref<EigenVec> &arm_reward,
            Eigen::Ref<EigenBoolVec> &done) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentStep(i, action, reward, arm_reward, done);
  }
  ...
  void isTerminalState(Eigen::Ref<EigenBoolVec>& terminalState) {
    for (int i = 0; i < num_envs_; i++) {
      float terminalReward, arm_terminalReward;
      terminalState[i] = environments_[i]->isTerminalState(terminalReward, arm_terminalReward);
    }
  }
  ...
  const std::vector<std::map<std::string, float>>& getarmRewardInfo() { return armrewardInformation_; }
...
#pragma omp parallel for schedule(auto)
  ...
  inline void perAgentStep(int agentId,
                           ...
                           Eigen::Ref<EigenVec> &arm_reward,
                           Eigen::Ref<EigenBoolVec> &done) {
    reward[agentId] = environments_[agentId]->step(action.row(agentId))[0];
    arm_reward[agentId] = environments_[agentId]->step(action.row(agentId))[1];
    rewardInformation_[agentId] = environments_[agentId]->getRewards().getStdMap();
    armrewardInformation_[agentId] = environments_[agentId]->getarmRewards().getStdMap();

    float terminalReward = 0, arm_terminalReward = 0;
    done[agentId] = environments_[agentId]->isTerminalState(terminalReward, arm_terminalReward);

    if (done[agentId]) {
      environments_[agentId]->reset();
      reward[agentId] += terminalReward;
      arm_reward[agentId] += arm_terminalReward;
    }
  }

  std::vector<ChildEnvironment *> environments_;
  std::vector<std::map<std::string, float>> rewardInformation_;
  std::vector<std::map<std::string, float>> armrewardInformation_;
