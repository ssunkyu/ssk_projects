...
class PPO:
    def __init__(self,
        ...
        self.counter = 0
        self.value_mixing_ratio = 0

    def act(self, actor_obs):
        self.actor_obs = actor_obs
        with torch.no_grad():
            self.actions, self.actions_log_prob = self.actor.sample(torch.from_numpy(actor_obs).to(self.device))
            self.attentions = self.critic.attention(torch.from_numpy(actor_obs).to(self.device))
        return self.actions, self.attentions
    ...
    def update(self, actor_obs, value_obs, log_this_iteration, update):
        self.value_mixing_ratio = self.get_value_mixing_ratio()
        last_values = self.critic.predict(torch.from_numpy(value_obs).to(self.device))[:,1].unsqueeze(dim=-1)
    ...
    def _train_step(self, log_this_iteration):
        ...
        for epoch in range(self.num_learning_epochs):
            for actor_obs_batch, critic_obs_batch, actions_batch, old_sigma_batch, old_mu_batch, current_values_batch, advantages_batch, returns_batch, old_actions_log_prob_batch \
                ...
                value_batch = self.critic.evaluate(critic_obs_batch)[:,0].unsqueeze(dim=-1)
