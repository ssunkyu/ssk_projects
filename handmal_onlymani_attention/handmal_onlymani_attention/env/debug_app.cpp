...
#include "Attention_Environment.hpp"
...
int main(int argc, char *argv[]) {
  ...
  EigenRowMajorMat attention(config["num_envs"].template As<int>(), vecEnv.getRewDim());
  EigenVec reward(config["num_envs"].template As<int>(), 1);
  EigenBoolVec dones(config["num_envs"].template As<int>(), 1);
  action.setZero();
  attention.setZero();

  Eigen::Ref<EigenRowMajorMat> ob_ref(observation), action_ref(action), attention_ref(attention);
  Eigen::Ref<EigenVec> reward_ref(reward);
  Eigen::Ref<EigenBoolVec> dones_ref(dones);

  vecEnv.reset();
  vecEnv.step(action_ref, attention_ref, reward_ref, dones_ref);
