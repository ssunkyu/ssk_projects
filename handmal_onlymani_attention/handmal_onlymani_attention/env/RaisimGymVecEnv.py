...
class RaisimGymVecEnv:
    ...
    def __init__(self, impl, normalize_ob=True, seed=0, clip_obs=10.):
        ...
        self.num_rews = self.wrapper.getRewDim()
        ...
        self.torque = np.zeros(self.num_acts, dtype=np.float32)
        self.eef_cmd = np.zeros(2, dtype=np.float32)
    ...
    def step(self, action, attention):
        self.wrapper.step(action, attention, self._reward, self._done)
    ...
    def getTorque(self):
        self.wrapper.getTorque(self.torque)
        return self.torque
    ...
    def setCommand(self):
        ...
        self.eef_cmd = np.array(temp_list, dtype=np.float32).reshape(-1, 1)
        self.wrapper.setCommand(self.eef_cmd)
