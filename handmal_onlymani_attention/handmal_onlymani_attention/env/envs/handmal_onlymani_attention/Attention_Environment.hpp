...
//            pTarget15_ = action.cast<double>().cwiseProduct(actionStd_) + gc_init_.tail(15);
            ...
            for (int fi = 0; fi < nFoot_; fi++){
                rewards_.record("airtime",
                                attention_[0] * std::clamp(t_s_[fi] - t_a_[fi],-0.3,0.3),true);
                rewards_.record("footclearance",
                                attention_[1] * pow((foot_pos_height_[fi]-clearance_),2) * pow(foot_vel_[fi],0.25),true); // clearance = 0.07
                rewards_.record("footslip",
                                attention_[2] * grf_bin_[fi] * foot_vel_[fi],true);
            }

            // positive rewards
            if(step_counter_) {
                rewards_.record("cmdposition",
                                attention_[3] * exp(-((ee_curr_commnad_[step_counter_ - 1].head(3) - ee_position_).squaredNorm()/pow(0.04/cf_,2))));
            }
