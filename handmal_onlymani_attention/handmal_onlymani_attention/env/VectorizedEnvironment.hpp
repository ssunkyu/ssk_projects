...
class VectorizedEnvironment {
 ...
 public:
  ...
  void init() {
    ...
    rewDim_ = environments_[0]->getRewDim();
...
#pragma omp parallel for schedule(auto)
  ...
  void step(Eigen::Ref<EigenRowMajorMat> &action,
            Eigen::Ref<EigenRowMajorMat> &attention,
            Eigen::Ref<EigenVec> &reward,
            Eigen::Ref<EigenBoolVec> &done) {
#pragma omp parallel for schedule(auto)
    for (int i = 0; i < num_envs_; i++)
      perAgentStep(i, action, attention, reward, done);
  }
  ...
  void getTorque(Eigen::Ref<EigenVec> torque) {
    environments_[0]->getTorque(torque);
  }
...
#pragma omp parallel for schedule(auto)
  ...
  inline void perAgentStep(int agentId,
                           ...
                           Eigen::Ref<EigenVec> &reward,
                           Eigen::Ref<EigenBoolVec> &done) {
    reward[agentId] = environments_[agentId]->step(action.row(agentId), attention.row(agentId));
  ...
  int num_envs_ = 1;
  int itr_number = 0;
  int obDim_ = 0, actionDim_ = 0, rewDim_ = 0;
